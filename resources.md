### AWS Cloud Shell
* https://docs.aws.amazon.com/cloudshell/latest/userguide/welcome.html

___

### Visual Studio Code

* [Setting up Visual Studio Code for PowerShell Development - Trevor Sullivan](https://www.youtube.com/watch?v=LJNdK0QrIo8&t=2s)

___

### Learning PowerShell

- [PowerShell in a month of lunches book](https://www.manning.com/books/learn-windows-powershell-in-a-month-of-lunches-third-edition)
- [Microsoft Learn site](https://docs.microsoft.com/en-us/powershell/scripting/learn/understanding-important-powershell-concepts?view=powershell-7)
- [Jeff Snover & Jason Helmick](https://channel9.msdn.com/Series/Getting-Started-with-Microsoft-PowerShell)
- [John Saville's MasterClass](https://www.youtube.com/watch?v=sQm4zRvvX58&list=PLlVtbbG169nFq_hR7FcMYg32xsSAObuq8&index=1)

___

### AWS PowerShell

#### Reference

* [AWS Tools for PowerShell Documentation](https://docs.aws.amazon.com/powershell/?id=docs_gateway)
* [AWS Tools for PowerShell User Guide](https://docs.aws.amazon.com/powershell/latest/userguide/)
* [AWS Tools for PowerShell Cmdlet Reference](https://docs.aws.amazon.com/powershell/latest/reference/)
* [AWS Tools for PowerShell GitHub Repo](https://github.com/aws/aws-tools-for-powershell)
* [AWS SDK for .NET Version 3 API Reference](https://docs.aws.amazon.com/sdkfornet/v3/apidocs/)


#### Samples

* [AWS PowerShell code samples](https://aws.amazon.com/developer/language/net/code-samples/powershell-code-samples/)
* [AWS Samples for PowerShell on GitHub](https://github.com/aws-samples?q=&type=&language=powershell&sort=)
* [Working with AWS and Powershell](https://www.smartfile.com/blog/working-aws-powershell/)

#### Tutorials

* [Tutorial: Getting started with AWS CloudShell](https://docs.aws.amazon.com/cloudshell/latest/userguide/getting-started.html)
* [Cloudopian AWS PowerShell tutorial](https://cloudopian.com/blog/aws-powershell-tutorial/)

#### Cool AWS Prompt!

* [AwsPsPrompt - Shows AWS region and profile in prompt](https://github.com/aws-samples/eks-powershell-shortcuts/blob/710c7929aecc23d2f638394f7ce63ec02ff123e8/src/AwsPsPrompt.ps1)
